...
RSpec.describe EmptyLocationForm, type: :model do
  ...
  let(:params)                      do
    ActionController::Parameters.new(controller: 'empty_locations', action: 'create')
  end
  ...
  it 'will not be valid if the locations has child locations' do
    ...
    expect(new_form.errors.full_messages)
      .to include("Location with barcode #{location_with_children.barcode} has child locations")
  end
