...
RSpec.describe LabelPrinter, type: :model do
  ...
  it 'if the printing is successful should return an appropriate message' do
    ...
    label_printer = LabelPrinter.new(printer: printer.id, locations: location.id, label_template_name: 'labwhere_1d',
                                     copies: 1)
  ...
  it 'should print multiple labels' do
    label_printer = LabelPrinter.new(printer: printer.id, locations: locations.map(&:id),
  ...
  it 'should print multiple copies' do
    label_printer = LabelPrinter.new(printer: printer.id, locations: locations.map(&:id),
                                     label_template_name: 'labwhere_1d', copies: 2)
