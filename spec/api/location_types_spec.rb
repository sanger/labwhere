...
RSpec.describe Api::LocationTypesController, type: :request do
  let!(:swipe_card_id) { "SwipeCardId:10" }
  let!(:administrator) { create(:administrator, swipe_card_id: swipe_card_id) }
  ...
  it "should create a new location type" do
    post api_location_types_path, params: { location_type: attributes_for(:location_type).merge(user_code: swipe_card_id) }
  ...
  it "should return an error if the location type has invalid attributes" do
    post api_location_types_path, params: { location_type: attributes_for(:location_type).except(:name).merge(user_code: swipe_card_id) }
  ...
  it "should update an existing location type" do
    ...
    patch api_location_type_path(location_type), params: { location_type: { user_code: swipe_card_id, name: location_type_new.name } }
    expect(response).to be_successful
  ...
  it "should return an error if the updated location type has invalid attributes" do
    ...
    patch api_location_type_path(location_type_1), params: { location_type: { user_code: swipe_card_id, name: location_type_2.name } }
    expect(response).to have_http_status(:unprocessable_entity)
    expect(ActiveSupport::JSON.decode(response.body)["errors"]).not_to be_empty
  end
end
