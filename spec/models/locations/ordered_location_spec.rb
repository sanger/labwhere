...
RSpec.describe OrderedLocation, type: :model do
  it '#populate should create correct number of coordinates from rows and columns' do
    location1 = create(:ordered_location_with_parent, rows: 3, columns: 4)
    location2 = create(:ordered_location_with_parent, rows: 1, columns: 1)

    Hash.grid(3, 4) do |pos, row, col|
      expect(location1.coordinates.find_by_position(position: pos)).to_not be_nil
      expect(location1.coordinates.find_by_position(position: pos)).to eq(location1.coordinates.find_by_position(
                                                                            row: row, column: col
                                                                          ))
    end

    expect(location2.coordinates.find_by_position(row: 2, column: 2)).to be_nil
  end
