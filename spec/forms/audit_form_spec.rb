...
RSpec.describe AuditForm, type: :model do 
  ...
  it "allows creation of new record with valid attributes" do
    expect{ 
      TestModelForm.new.submit(params.merge(test_model: valid_params.merge(user: admin_user.swipe_card_id)))
  ...
  it "prevents creation of new record with invalid attributes" do
    ...
    expect{ 
      test_model_form.submit(params.merge(test_model: valid_params.merge(name: nil, user: admin_user.swipe_card_id)))
  ...
  it "allows updating of existing record with valid attributes" do
    ...
    expect{
      test_model_form.submit(params.merge(test_model: valid_params.merge(name: "Another name", user: admin_user.barcode)))
  ...
  it "should reject modification of the record if the user is unknown" do
    ...
    expect(test_model_form.errors.full_messages).to include("User does not exist")
  end

  it "should reject modification of the record if the user does not have authorisation" do
    test_model_form = TestModelForm.new
    expect{ 
      test_model_form.submit(params.merge(test_model: valid_params.merge(user: standard_user.swipe_card_id)))
    }.to_not change(TestModel, :count)
    expect(test_model_form.errors.full_messages).to include("User is not authorised")
  end
