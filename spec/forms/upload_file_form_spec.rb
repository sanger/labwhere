# frozen_string_literal: true

require 'rails_helper'

RSpec.describe UploadFileForm, type: :model do
  let(:create_upload_file)    { UploadFileForm.new }
  let!(:user)                 { create(:scientist) }
  let(:params)                { ActionController::Parameters.new(controller: 'upload_file', action: 'create') }
  let(:file_param)            { ActionDispatch::Http::UploadedFile.new(tempfile: tempfile) }
  let(:tempfile)              { Tempfile.new(['foo', '.csv']) }

  before do
    file_param.content_type = 'text/csv' # doesn't work if set it in the initializer
  end

  it 'will not be valid without the user barcode filled in' do
    create_upload_file.submit(params.merge(upload_file_form:
      { 'file' => file_param }))
    expect(create_upload_file.errors.full_messages).to include('The required fields must be filled in')
  end

  it 'will not be valid without a file selected' do
    create_upload_file.submit(params.merge(upload_file_form:
      { 'user_code' => user.barcode }))
    expect(create_upload_file.errors.full_messages).to include('The required fields must be filled in')
  end

  it 'will not be valid without a valid user' do
    create_upload_file.submit(params.merge(upload_file_form:
      { 'user_code' => 'dummy_user_code', 'file' => file_param }))
    expect(create_upload_file.errors.full_messages).to include("User #{I18n.t('errors.messages.existence')}")
  end

  it 'will not be valid with the wrong format of file' do
    create_upload_file.submit(params.merge(upload_file_form:
      { 'user_code' => user.barcode, 'file' => 'dummy_file' }))
    expect(create_upload_file.errors.full_messages).to include('File must be a csv')
  end

  it 'will be valid with all its params and a valid user' do
    create_upload_file.submit(params.merge(upload_file_form:
      { 'user_code' => user.barcode, 'file' => file_param }))
    expect(create_upload_file.valid?).to eq(true)
  end
end
