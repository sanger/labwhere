...
RSpec.describe LabwareCollection, type: :model do
  ...
  describe 'adding to an unordered location' do
    ...
    let(:labware_collection)  do
      LabwareCollection.open(location: location, user: administrator, labwares: labwares.extract_barcodes)
    end
    ...
    it 'provides a list of the original locations for the labware' do
      labware_collection.push
      expect(labware_collection.original_location_names)
        .to eq("#{previous_location1.name}, #{previous_location2.name}")
    end
    ...
    it 'removes duplicates from labwares which have been passed' do
      labware_collection = LabwareCollection.open(location: location, user: administrator,
                                                  labwares:
                                                  new_labwares.extract_barcodes + new_labwares.extract_barcodes)
  ...
  describe 'adding to an ordered location with a starting position' do
    ...
    it 'adds labwares to the correct coordinates if a starting position is added' do
      labware_collection = LabwareCollection.open(location: location, user: administrator,
                                                  labwares: labwares.extract_barcodes, start_position: 5)
      labware_collection.push
      expect(location.labwares.count).to eq(labware_collection.count)
      ((labware_collection.start_position)..(labware_collection.start_position
                                             + labware_collection.count - 1)).each do |i|
    ...
    it 'allows coordinates to be passed' do
      ...
      labware_collection = LabwareCollection.open(location: location, user: administrator,
                                                  labwares: labwares.extract_barcodes, coordinates: coordinates)
      labware_collection.push
      expect(location.labwares.count).to eq(labware_collection.count)
    end
    ...
    it 'fails if the position runs over the end of the available coordinates' do
      labware_collection = LabwareCollection.open(location: location, user: administrator,
                                                  labwares: labwares.extract_barcodes, start_position: 20)
    ...
    it 'removes duplicates from labwares which have been passed' do
      labware_collection = LabwareCollection.open(location: location, user: administrator,
                                                  labwares: new_labwares.extract_barcodes +
                                                  new_labwares.extract_barcodes,
                                                  start_position: 5)
      expect(labware_collection.count).to eq(new_labwares.count)
      labware_collection.push
      ((labware_collection.start_position)..
      (labware_collection.start_position + labware_collection.count - 1)).each do |i|
        expect(location.coordinates.find_by_position(position: i)).to be_filled
      end
    end
  end
