...
RSpec.describe MoveLocationForm, type: :model do
  ...
  it 'will not be valid without a valid user' do
    create_move_location.submit(params.merge(move_location_form:
      { 'parent_location_barcode' => parent_location.barcode,
        'child_location_barcodes' => child_locations.join_barcodes }))
    expect(create_move_location.errors.full_messages)
      .to include("User #{I18n.t('errors.messages.existence')}")
  end

  it 'will not be valid without a location' do
    create_move_location.submit(params.merge(move_location_form:
      { 'parent_location_barcode' => 'lw-no-location-here', 'child_location_barcodes' => child_locations.join_barcodes,
        'user_code' => tech_swipe_card_id }))
    expect(create_move_location.errors.full_messages)
      .to include("Parent location #{I18n.t('errors.messages.existence')}")
  end
  ...
  it 'will not be valid if the location is not a container' do
    ...
    create_move_location.submit(params.merge(move_location_form:
      { 'parent_location_barcode' => building_location.barcode,
        'child_location_barcodes' => child_locations.join_barcodes, 'user_code' => tech_swipe_card_id }))
  ...
  it 'will not be valid if the child location is protected and user is scientist' do
    ...
    create_move_location.submit(params.merge(move_location_form:
      { 'parent_location_barcode' => parent_location.barcode, 'child_location_barcodes' => protected_location.barcode,
        'user_code' => sci_swipe_card_id }))
    expect(create_move_location.errors.full_messages)
      .to include("Location with barcode #{protected_location.barcode} #{I18n.t('errors.messages.protected')}")
  end
  ...
  it 'will not be valid unless all of the child locations exist' do
    ...
    expect(create_move_location.errors.full_messages)
      .to include("Location with barcode lw-no-location-here #{I18n.t('errors.messages.existence')}")
  end
  ...
  describe 'duplicate child location barcodes' do
    it 'will remove duplication child location barcodes' do
      ...
      create_move_location.submit(params.merge(move_location_form:
      { 'parent_location_barcode' => parent_location.barcode, 'child_location_barcodes' => duplicate_barcodes,
        'user_code' => tech_swipe_card_id }))
