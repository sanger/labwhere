...
RSpec.describe Api::LocationsController, type: :request do
  ...
  it 'should return an error if the location has invalid attributes' do
    post api_locations_path,
         params: { location: attributes_for(:location).except(:name).merge(user_code: admin_swipe_card_id) }
  ...
  it 'should update an existing location' do
    ...
    patch api_location_path(location.barcode),
          params: { location: { user_code: admin_swipe_card_id, parent_id: location_parent.id } }
    expect(response).to be_successful
