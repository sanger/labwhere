...
RSpec.describe FormObject::FormVariables, type: :model do
  let(:form_variables)  { FormObject::FormVariables.new(ALovelyModel, nil, %i[controller action]) }

  let(:params)          do
    { controller: 'controller', action: 'action', a_lovely_model: { attr_a: 'a', attr_b: 'b', attr_d: 'd' } }
  end
